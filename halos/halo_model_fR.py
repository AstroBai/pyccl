...
class HMCalculator(CCLAutoRepr):
    ...
    def _get_mass_function(self, cosmo, a, rho0):
        ...
        if a != self._a_mf or cosmo != self._cosmo_mf:
            ...
            ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            """
            emu = hmf_global.emulator(model_name="defgrs_SB7_ann")
            h = cosmo['h']
            mnu = np.sum(cosmo['m_nu'])
            Onu = mnu/ 93.14 / h**2
            emu.set_cosmo(Om=cosmo['Omega_c'] + cosmo['Omega_b'] + Onu,
                           Ob=cosmo['Omega_b'],
                           h=cosmo['h'],
                           ns=cosmo['n_s'],
                           As=cosmo['A_s'],
            ...
            mass_hubble = self._mass * h
            self._mf = emu.get_hmf(mass=mass_hubble, z=z) * self._mass * h**4 * np.log(10)
            #print(self._mf)
